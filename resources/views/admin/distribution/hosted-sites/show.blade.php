    ...
    <div class="space-y-7 px-4 sm:px-0">
        ...
        <section class="border-y border-gray-200 bg-white px-5 py-6 sm:px-7">
            <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div class="flex flex-wrap gap-2">
                    <form method="POST" action="{{ route('admin.distribution.hosted-sites.activate', $channel) }}">@csrf<button class="min-h-10 rounded-md bg-emerald-600 px-4 py-2 text-sm font-medium text-white transition-transform active:scale-[.96] hover:bg-emerald-700">激活</button></form>
                    <form method="POST" action="{{ route('admin.distribution.hosted-sites.pause', $channel) }}">@csrf<button class="min-h-10 rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition-transform active:scale-[.96] hover:bg-gray-50">暂停接收</button></form>
                    <form method="POST" action="{{ route('admin.distribution.hosted-sites.maintenance', $channel) }}">@csrf<button class="min-h-10 rounded-md bg-amber-600 px-4 py-2 text-sm font-medium text-white transition-transform active:scale-[.96] hover:bg-amber-700">进入维护</button></form>
                </div>
            </div>
